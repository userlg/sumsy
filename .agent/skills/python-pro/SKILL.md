...
- **Cross-Platform Persistence**: Correct usage of `AppData` (Windows) and `XDG_DATA_HOME` (Linux) for configs/logs.
- **Resource Management**: Handling relative paths in both development (`__file__`) and frozen (`sys._MEIPASS`) environments.
- **GUI Frameworks**: Best practices for PySide6/PyQt6 (threading, signals, styling).
- **Audio/Video Processing**: Real-time handling with `sounddevice`, `numpy`, and `opencv`.
